...
3. 로그인 필요한 사이트라면 `ID`, `PW`를 입력하거나 `Session`에 쿠키 값을 입력합니다.
4. `캡처 시작`을 누르면 서버가 Playwright 브라우저를 열고 대상 URL로 이동합니다.
5. `Session` 값이 있으면 쿠키로 먼저 적용하고, 없으면 ID/PW로 첫 화면의 로그인 폼을 자동 감지해 입력/제출합니다.
...
- `ID`, `PW`, `Session`은 인증 자동화에만 사용하며 앱 저장소에는 저장하지 않습니다.
- `Session`은 `SESSIONID=...` 또는 `SESSIONID=...; other=...` 같은 Cookie 문자열을 권장합니다. 값만 넣으면 `session=<값>` 쿠키로 적용합니다.
...
- `FIREBASE_SERVICE_ACCOUNT_JSON`과 `CLOUDFLARE_R2_SECRET_ACCESS_KEY`는 서버 전용입니다.
